...
"@
    ...
    [AmpRequest] GetRequestById([string]$Id) {
        [string]$daprHttpPort = $env:DAPR_HTTP_PORT
        [string]$stateStoreName = 'dapr-conf-submit-queue-state'

        Write-Host "dapr port environment variable is: $($daprHttpPort)"

        $RestParams = @{
            Method      = 'Get'
            Uri         = "http://localhost:$($daprHttpPort)/v1.0/state/$($stateStoreName)/$($Id)"
            ErrorAction = 'Stop'
        }
        ...
        try {
            $response = Invoke-RestMethod @RestParams
            if ($null -eq $response) {
                throw "Request with ID '$Id' not found."
            }
            return [AmpRequest]::new($response)
        }
        catch {
            Write-Error $_.Exception.Message
            return $null
